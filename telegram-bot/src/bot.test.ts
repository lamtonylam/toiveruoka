...
describe('Bot Commands Configuration', () => {
  ...
  it('calls setMyCommands and setChatMenuButton on setupBotCommands', async () => {
    ...
    expect(setMyCommandsMock).toHaveBeenCalledWith(BOT_COMMANDS_FI, {
      language_code: 'fi',
    })
    expect(setMyCommandsMock).toHaveBeenCalledWith(BOT_COMMANDS_EN, {
      language_code: 'en',
    })
...
describe('replySplitMessage', () => {
  it('sends single message when content is within limit', async () => {
    ...
    expect(replyMock).toHaveBeenCalledWith('Hello world', {
      parse_mode: 'HTML',
    })
  })
  ...
  it('splits long content exceeding 4000 characters into multiple messages', async () => {
    ...
    expect(replyMock).toHaveBeenNthCalledWith(1, block1, {
      parse_mode: 'HTML',
    })
    expect(replyMock).toHaveBeenNthCalledWith(2, block2, {
      parse_mode: 'HTML',
    })
  })
})
...
describe('Alert Preferences & Subscriber Filtering', () => {
  ...
  it('getAllSubscribers excludes users who have opted out of alerts', async () => {
    ...
    jest
      ...
      .mockResolvedValue([{ chatId: BigInt(100), alertsEnabled: false }])
