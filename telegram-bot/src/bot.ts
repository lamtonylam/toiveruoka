...
export const mainMenu = new Menu('main-menu')
  ...
  .text('Omat valinnat', async (ctx) => {
    ...
    summary += activeFoods.length
      ? activeFoods.map((f) => `• ${f}`).join('\n')
      : '<i>Ei vielä valittuja ruokia</i>'
    summary += `\n\n<b>Ravintolat (${activeRestos.length}):</b>\n`
    summary += activeRestos.length
      ? activeRestos.map((r) => `• ${r}`).join('\n')
...
export const BOT_COMMANDS = [
  ...
  {
    command: 'checkall',
    description: 'Tarkista tulevat lounaat tai hae ruokaa',
  },
...
export function createBot(token: string): Bot {
  ...
  bot.command(['alerts', 'tilaus'], async (ctx) => {
    ...
    if (
      ...
      await ctx.reply('Aamuilmoitukset (klo 10:00): <b>Päällä</b>', {
        parse_mode: 'HTML',
      })
    } else if (
      arg === 'off' ||
      ...
      await ctx.reply('Aamuilmoitukset (klo 10:00): <b>Pois päältä</b>', {
        parse_mode: 'HTML',
      })
    } else {
  ...
  bot.command('addfood', async (ctx) => {
    ...
    if (!food) {
      await ctx.reply('Anna ruoan nimi: <code>/addfood Lohikeitto</code>', {
        parse_mode: 'HTML',
      })
      return
    }
    if (ctx.chat?.id) {
      ...
      await ctx.reply(`Lisätty <b>${food}</b> listallesi!`, {
        parse_mode: 'HTML',
      })
    }
  })
  ...
  bot.command('removefood', async (ctx) => {
    const food = ctx.match?.trim()
    if (!food) {
      await ctx.reply('Anna poistettavan ruoan nimi: <code>/removefood Kalapuikot</code>', {
        parse_mode: 'HTML',
      })
      return
    }
    if (ctx.chat?.id) {
      ...
      await ctx.reply(`Poistettu <b>${food}</b> listaltasi.`, {
        parse_mode: 'HTML',
      })
    }
  })
...
export async function performLunchCheck(ctx: Context, chatId: number): Promise<void> {
  ...
  try {
    ...
    await ctx.reply(
      'Menujen haku backend-rajapinnasta epäonnistui. Yritä hetken kuluttua uudelleen.'
    )
  }
}
 ...
 */
export async function startBotWithRetry(bot: Bot, options?: BotRunnerOptions): Promise<void> {
  ...
  while (!signal?.aborted) {
    try {
      ...
      const grammyErr = err as GrammyError | undefined
      const isConflict =
        (grammyErr instanceof GrammyError &&
          (grammyErr as { error_code?: number }).error_code === 409) ||
        (err &&
          typeof err === 'object' &&
          'error_code' in err &&
        ...
        (err &&
          typeof err === 'object' &&
          'description' in err &&
          typeof (err as { description: string }).description === 'string' &&
          (err as { description: string }).description.includes('Conflict')) ||
        (err &&
          typeof err === 'object' &&
          'message' in err &&
      ...
      if (isConflict) {
        if (options?.onConflict) {
          ...
          console.warn('[Bot] Note: Telegram only allows ONE running bot instance per token.')
          console.warn(
